...
void WebSocket<isServer>::send(const char *message, size_t length, OpCode opCode, void(*callback)(WebSocket<isServer> *webSocket, void *data, bool cancelled, void *reserved), void *callbackData) {
    ...
    struct WebSocketTransformer {
        static size_t estimate(const char *, size_t length) {
 ...
 */
template <bool isServer>
typename WebSocket<isServer>::PreparedMessage *WebSocket<isServer>::prepareMessageBatch(std::vector<std::string> &messages, std::vector<int> &, OpCode opCode, bool compressed, void (*callback)(WebSocket<isServer> *, void *, bool, void *))
{
    // should be sent in!
...
void WebSocket<isServer>::close(int code, const char *message, size_t length) {
    ...
    send(closePayload, closePayloadLength, OpCode::CLOSE, [](WebSocket<isServer> *p, void *, bool cancelled, void *) {
        if (!cancelled) {
            p->shutdown();
        }
    });
