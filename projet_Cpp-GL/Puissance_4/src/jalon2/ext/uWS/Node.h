...
protected:
    ...
    static void connect_cb(Poll *p, int status, int ) {
        C((Socket *) p, status < 0);
    }

    template <void A(Socket *s)>
    static void accept_poll_cb(Poll *p, int , int ) {
...
public:
    ...
    template <void A(Socket *s)>
    bool listen(const char *host, int port, uS::TLS::Context sslContext, int options, uS::NodeData *nodeData, void *) {
        addrinfo hints, *result;
        memset(&hints, 0, sizeof(addrinfo));
